...
| 用户提到"手绘风格""Excalidraw""马克笔风格""水彩涂鸦""技术PPT风格"等风格关键词 | → 直接跳对应的 `references/styles/*.md`（见下方风格库表） |
...
- 风格参考图库：`assets/styles/`（`handdrawn/` 手绘插画、`excalidraw/` 白板斜线填充、`mono-marker/` 单色马克笔、`doodle-watercolor/` 水彩涂鸦、`techppt/` 技术PPT风格，配合 `--ref` 使用，对应文档见 `references/styles/`）
